...
RapidDecay* RapidConfig::getDecay() {
  if ( !decay_ ) {
    ...
    if ( external_ ) { decay_->setExternal( external_ ); }
  }
...
RapidHistWriter* RapidConfig::getWriter( bool saveTree ) {
  ...
  if ( !writer_ ) {
    ...
    writer_ = new RapidHistWriter( parts_, params_, paramsStable_, paramsDecaying_, paramsTwoBody_, paramsThreeBody_,
                                   histFileName, saveTree );
  }
...
bool RapidConfig::loadDecay() {
  std::cout << "INFO in RapidConfig::loadDecay : loading decay descriptor from file: " << fileName_ << ".decay"
            << std::endl;
  TString                    decayStr;
...
bool RapidConfig::loadConfig() {
  std::cout << "INFO in RapidConfig::loadConfig : attempting to load configuration from file: " << fileName_ + ".config"
            << std::endl;
  ...
  if ( !fin.good() ) {
    std::cout << "INFO in RapidConfig::loadConfig : failed to load configuration. Will write default config file to: "
              << fileName_ + ".config" << std::endl;
    fin.close();
    writeConfig();
    fin.open( fileName_ + ".config", std::ifstream::in );
  }
...
void RapidConfig::writeConfig() {
  ...
  for ( unsigned int i = 0; i < parts_.size(); ++i ) {
    ...
    if ( part->nDaughters() == 0 ) {
      ...
      if ( TMath::Abs( part->id() ) == 12 || TMath::Abs( part->id() ) == 14 || TMath::Abs( part->id() ) == 16 ) {
        fout << "\tinvisible : true\n";
      }
    }
  }
...
bool RapidConfig::configParticle( unsigned int part, TString command, TString value ) {
  ...
  if ( command == "name" ) {
    ...
    while ( value.Tokenize( buffer, from, " " ) ) {
      ...
      if ( altId == 0 ) {
        std::cout << "WARNING in RapidConfig::configParticle : unknown particle type, " << buffer
                  << ", in alternative mass hypothesis for " << parts_[part]->name() << std::endl;
      ...
      std::cout << "INFO in RapidConfig::configParticle : added alternative mass hypothesis of " << buffer
                << " for particle " << parts_[part]->name() << std::endl;
    }
  } else if ( command == "evtGenModel" ) {
    parts_[part]->setEvtGenDecayModel( value );
    std::cout << "INFO in RapidConfig::configParticle : set EvtGen decay model for particle " << parts_[part]->name()
              << std::endl
              << "                                    : " << value << std::endl;
  }
...
bool RapidConfig::configGlobal( TString command, TString value ) {
  if ( command == "seed" ) {
    ...
    std::cout << "INFO in RapidConfig::configGlobal : setting seed for random number generation to " << seed << "."
              << std::endl
    ...
    std::cout << "INFO in RapidConfig::configGlobal : parent flavour forced to be " << motherFlavour_ << "."
              << std::endl;
  } else if ( command == "minWidth" ) {
    RapidParticleData::getInstance()->setNarrowWidth( value.Atof() );
    std::cout << "INFO in RapidConfig::configGlobal : minimum resonance width to be generated set to " << value.Atof()
              << " GeV." << std::endl;
  } else if ( command == "maxAttempts" ) {
    maxgen_ = value.Atof();
    std::cout << "INFO in RapidConfig::configGlobal : maximum number of attempts to generated an event set to "
              << maxgen_ << "." << std::endl;
  } else if ( command == "paramsStable" ) {
    paramStrStable_ = value;
    std::cout << "INFO in RapidConfig::configGlobal : will use the following parameters for all stable particles:"
              << std::endl
              << "                                    " << paramStrStable_ << "." << std::endl;
  } else if ( command == "paramsDecaying" ) {
    paramStrDecaying_ = value;
    std::cout << "INFO in RapidConfig::configGlobal : will use the following parameters for all decaying particles:"
              << std::endl
              << "                                    " << paramStrDecaying_ << "." << std::endl;
  } else if ( command == "paramsTwoBody" ) {
    paramStrTwoBody_ = value;
    std::cout << "INFO in RapidConfig::configGlobal : will use the following parameters for all two-body daughter "
                 "combinations:"
              << std::endl
              << "                                    " << paramStrTwoBody_ << "." << std::endl;
  } else if ( command == "paramsThreeBody" ) {
    paramStrThreeBody_ = value;
    std::cout << "INFO in RapidConfig::configGlobal : will use the following parameters for all three-body daughter "
                 "combinations:"
              << std::endl
    ...
    if ( !getenv( "EVTGEN_ROOT" ) ) {
      std::cout << "ERROR in RapidConfig::configGlobal : EVTGEN_ROOT environment variable must be set to use external "
                   "EvtGen generator."
                << std::endl;
      return false;
    }
    external_ = new RapidExternalEvtGen();
    std::cout << "INFO in RapidConfig::configGlobal : will use external EvtGen generator to decay particles."
              << std::endl;
    ...
    if ( vptr == nullptr ) {
      ...
      if ( status == -1 )
        throw std::runtime_error( "RapidConfig::configGlobal : Directory " + outputDir_ +
                                  " can not be created. Please check path" );
    }
    delete vptr;
  }
  return true;
...
bool RapidConfig::loadRange( TString name, TString str, double& min, double& max ) {
  ...
  if ( str.Tokenize( buffer, from, " " ) ) {
    ...
    std::cout << "ERROR in RapidConfig::loadRange : failed to setup range \"" << name << "\" - no minimum given."
              << std::endl;
    return false;
  }
  if ( str.Tokenize( buffer, from, " " ) ) {
    ...
    std::cout << "ERROR in RapidConfig::loadRange : failed to setup range \"" << name << "\" - no maximum given."
              << std::endl;
    return false;
  }
  return true;
...
RapidParam* RapidConfig::loadParam( TString paramStr ) {
  ...
  while ( paramStr.Tokenize( buffer, from, " " ) ) {
    if ( buffer.IsDec() ) {
      ...
      std::cout << "WARNING in RapidConfig::loadParam : unknown argument " << buffer << "in parameter " << name << "."
                << std::endl
  ...
  if ( ( type == RapidParam::THETA || type == RapidParam::COSTHETA ) && partlist.size() < 2 ) {
    std::cout << "WARNING in RapidConfig::loadParam : (cos)theta parameter expects at least two particles, "
              << partlist.size() << "were given." << std::endl
              << "                                    parameter " << name << " has not been added." << std::endl;
    return 0;
  }
...
RapidCut* RapidConfig::loadCut( TString cutStr ) {
  ...
  if ( cutType == "min" ) {
    ...
    std::cout << "ERROR in RapidConfig::loadCut : failed to setup cut - unknown cut type " << cutType << "."
              << std::endl;
  }
...
bool RapidConfig::loadSmearing( TString category ) {
  ...
  if ( path != "" ) {
    ...
    if ( fin.good() ) {
      std::cout << "INFO in RapidConfig::loadSmearing : found smearing category " << category << " in RAPIDSIM_CONFIG."
                << std::endl;
      std::cout << "                                    this version will be used." << std::endl;
      found = true;
    } else {
      std::cout << "INFO in RapidConfig::loadSmearing : smearing category " << category
                << " not found in RAPIDSIM_CONFIG." << std::endl;
  ...
  if ( type == "GAUSS" ) {
    ...
    if ( hists.size() == 0 ) {
      std::cout << "WARNING in RapidConfig::loadSmearing : failed to load any histograms for smearing category "
                << category << std::endl;
      file->Close();
      fin.close();
      return false;
    ...
    std::cout << "WARNING in RapidConfig::loadSmearing : unknown smearing type. Category " << category << " not added."
              << std::endl;
    file->Close();
    fin.close();
    return false;
...
void RapidConfig::setSmearing( unsigned int particle, TString category ) {
  if ( particle > parts_.size() ) {
    std::cout << "WARNING in RapidConfig::setSmearing : particle " << particle
              << " does not exist - smearing functions not set." << std::endl;
    return;
  }
  if ( parts_[particle]->nDaughters() != 0 ) {
    std::cout << "WARNING in RapidConfig::setSmearing : particle " << particle
              << " is composite - smearing functions not set." << std::endl;
    return;
  }

  std::cout << "INFO in RapidConfig::setSmearing : setting smearing functions for particle " << particle
            << " (category: " << category << ")" << std::endl;

  bool loadedmomsmear = true;
  if ( !momSmearCategories_.count( category ) ) {
    if ( !loadSmearing( category ) ) {
      std::cout << "WARNING in RapidConfig::setSmearing : failed to load momentum smearing category " << category << "."
                << std::endl
                << "                                      smearing functions not set for particle " << particle << "."
                << std::endl;
  ...
  if ( loadedmomsmear ) { parts_[particle]->setSmearing( momSmearCategories_[category] ); }

  bool loadedipsmear = true;
  if ( !ipSmearCategories_.count( category ) ) {
    if ( !loadSmearing( category ) ) {
      std::cout << "WARNING in RapidConfig::setSmearing : failed to load IP smearing category " << category << "."
                << std::endl
                << "                                      smearing functions not set for particle " << particle << "."
                << std::endl;
      loadedipsmear = false;
  ...
  if ( loadedipsmear ) { parts_[particle]->setSmearing( ipSmearCategories_[category] ); }
...
bool RapidConfig::loadPID( TString category ) {
  ...
  if ( path != "" ) {
    ...
    if ( fin.good() ) {
      std::cout << "INFO in RapidConfig::loadPID : found pid category " << category << " in RAPIDSIM_CONFIG."
                << std::endl;
      std::cout << "                                    this version will be used." << std::endl;
      found = true;
    } else {
      std::cout << "INFO in RapidConfig::loadPID : pid category " << category << " not found in RAPIDSIM_CONFIG."
                << std::endl;
      std::cout << "                                    checking RAPIDSIM_ROOT." << std::endl;
      fin.close();
    }
  ...
  while ( fin.good() ) {
    ...
    while ( line.Tokenize( buffer, from ) ) {
      if ( buffer.Contains( ".root" ) && !fileLoaded ) {
        ...
        if ( buffer.BeginsWith( "/" ) || buffer.BeginsWith( "." ) )
          file = TFile::Open( buffer );
        else
      ...
      if ( fileLoaded && idLoaded && buffer.Contains( "Prob" ) ) {
        ...
        if ( pidHists_.find( type ) == pidHists_.end() ) { pidHists_[type] = new RapidPID( buffer ); }
        pidHists_[type]->addPID( id, hist );
      }
      if ( pidHists_.empty() ) {
        std::cout << "WARNING in RapidConfig::loadPID : failed to load any histograms for PID category " << category
                  << std::endl;
        file->Close();
        fin.close();
        return false;
...
bool RapidConfig::loadAcceptRejectHist( TString histFile, TString histName, RapidParam* paramX, RapidParam* paramY ) {
  ...
  if ( !file ) {
    std::cout << "WARNING in RapidConfig::loadAcceptRejectHist : could not open file " << histFile << "." << std::endl
              << "                                               path should be absolute or relative to '"
              << getenv( "PWD" ) << "'." << std::endl
  ...
  TH1* hist = dynamic_cast<TH1*>( file->Get( histName ) );
  if ( !hist ) {
    std::cout << "WARNING in RapidConfig::loadAcceptRejectHist : could not load histogram " << histName << "."
              << std::endl
              << "                                               accept/reject histogram not set." << std::endl;
    return false;
  }
  ...
  if ( paramX ) {
    if ( paramY ) {
      ...
      if ( !check2D( hist ) ) {
        std::cout << "ERROR in RapidConfig::loadAcceptRejectHist : two parameters provided for shape PDF but histogram "
                     "is neither TH2F nor TH2D."
                  << std::endl;
        return false;
      }
    } else {
      ...
      if ( !check1D( hist ) ) {
        std::cout << "ERROR in RapidConfig::loadAcceptRejectHist : one parameter provided for shape PDF but histogram "
                     "is neither TH1F nor TH1D."
                  << std::endl;
        return false;
      }
    }
...
bool RapidConfig::loadParentKinematics() {
  ...
  if ( path != "" ) {
    ...
    if ( file ) {
      std::cout << "INFO in RapidConfig::loadParentKinematics : found kinematics LHC" << motherFlavour_ << ppEnergy_
                << " in RAPIDSIM_CONFIG." << std::endl
                << "                                            this version will be used." << std::endl;
      found = true;
    } else {
      std::cout << "INFO in RapidConfig::loadParentKinematics : kinematics LHC" << motherFlavour_ << ppEnergy_
                << " not found in RAPIDSIM_CONFIG." << std::endl
                << "                                            checking RAPIDSIM_ROOT." << std::endl;
    }
  }
  ...
  if ( !found ) {
    ...
    if ( !file ) {
      std::cout << "ERROR in RapidConfig::loadParentKinematics : unknown kinematics " << motherFlavour_
                << "-quark from " << ppEnergy_ << " TeV pp collision." << std::endl
  ...
  if(!phiHisto || !check1D(phiHisto)) {
          std::cout << "ERROR in RapidConfig::loadParentKinematics : phi histogram is neither TH1F nor TH1D." <<
  ...
  */
  ...
  if ( phiHisto ) { phiHisto_ = reduceHistogram( phiHisto, 0, 2 * TMath::Pi() ); }
...
void RapidConfig::setupDefaultParams() {
  ...
  while ( TString( paramStrStable_ ).Tokenize( buffer, from, " " ) ) {
    ...
    if ( type == RapidParam::UNKNOWN ) {
      std::cout << "WARNING in RapidConfig::setDefaultParams : Unknown parameter type " << buffer << "ignored."
                << std::endl;
      continue;
    } else {
      for ( unsigned int i = 0; i < parts_.size(); ++i ) {
        ...
        if ( part->nDaughters() == 0 ) {
          RapidPID* pidHists = 0;
          if ( pidHists_.find( type ) != pidHists_.end() && pidHists_[type] ) { pidHists = pidHists_[type]; }
    ...
    if ( type == RapidParam::UNKNOWN ) {
      ...
      for ( unsigned int i = 0; i < parts_.size(); ++i ) {
        ...
        if ( part->nDaughters() > 0 ) {
          ...
          std::cout << "DEBUG " << param->name() << " " << i << " " << param->eval() << " " << param->typeName() << " "
                    << type << std::endl;
          if ( param->canBeSmeared() ) {
            param->name();
...
TH1* RapidConfig::reduceHistogram( TH1* histo, double min, double max ) {
  ...
  if ( min < axis->GetXmin() ) {
    std::cout << "WARNING in RapidConfig::reduceHistogram : Minimum set to " << min << " but histogram "
              << histo->GetName() << " has minimum " << axis->GetXmin() << std::endl;
    std::cout << "                                        : Minimum changed to " << axis->GetXmin() << std::endl;
    min = axis->GetXmin();
  }
  if ( max > axis->GetXmax() ) {
    std::cout << "WARNING in RapidConfig::reduceHistogram : Maximum set to " << max << " but histogram "
              << histo->GetName() << " has maximum " << axis->GetXmax() << std::endl;
    std::cout << "                                        : Maximum changed to " << axis->GetXmax() << std::endl;
    max = axis->GetXmax();
  }

  // if we want the full histogram then just return it
  if ( min == axis->GetXmin() && max == axis->GetXmax() ) { return histo; }
  ...
  for ( int i = 1; i < nbinsOut; ++i ) { boundsOut[i] = bounds[firstBin + i - partFirstBin]; }
  ...
  for ( int i = 1; i <= nbinsOut; ++i ) {
    double value = histo->GetBinContent( firstBin + i - partFirstBin ) * histoOut->GetBinWidth( i ) /
                   histo->GetBinWidth( firstBin + i - partFirstBin );
