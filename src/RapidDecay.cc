...
void RapidDecay::setExternal( RapidExternalGenerator* external ) { external_ = external; }
  ...
  for ( unsigned int i = 0; i < parts_.size(); ++i ) {
    ...
    if ( part->nDaughters() > 0 ) {
      if ( !decay_.SetDecay( part->getP(), part->nDaughters(), part->daughterMasses() ) ) {
        std::cout << "ERROR in RapidDecay::checkDecay : decay of " << part->name() << " is kinematically forbidden."
                  << std::endl;
        return false;
      }
    }
...
bool RapidDecay::generate( bool genpar ) {
  ...
  bool decayed( false );
  if ( external_ ) { decayed = external_->decay( parts_ ); }
...
void RapidDecay::calcIPs() {
  ...
  for ( itrPart = parts_.begin(); itrPart != parts_.end(); ++itrPart ) {
    ...
    for ( itrVtx = pileuppvs_.begin(); itrVtx != pileuppvs_.end(); ++itrVtx ) {
      double thisip =
          getParticleIP( ( *itrVtx ).getVertex( true ), part->getOriginVertex()->getVertex( true ), part->getP() );
...
void RapidDecay::setup() {
  ...
  for ( unsigned int i = 0; i < parts_.size(); ++i ) { parts_[i]->print( i ); }
}

void RapidDecay::floatMasses() {
  for ( unsigned int i = 0; i < parts_.size(); ++i ) { parts_[i]->floatMass(); }
}

void RapidDecay::setupMasses() {
  RapidParticleData* particleData = RapidParticleData::getInstance();

  for ( unsigned int i = 0; i < parts_.size(); ++i ) { particleData->setupMass( parts_[i] ); }
}

bool RapidDecay::runAcceptReject() {
  if ( accRejParameterY_ )
    return runAcceptReject2D();
  else
    return runAcceptReject1D();
}
...
TH1* RapidDecay::generateAccRejDenominator1D() {
  ...
  //   std::cout << "accRejHisto_->GetName() = " << accRejHisto_->GetName() << "\n";
  //   std::cout << "accRejParameterX_->name() = " << accRejParameterX_->name()
  //             << " -  accRejParameterX_->truth() = " << accRejParameterX_->truth() << "\n";

  std::cout << "INFO in RapidDecay::generateAccRejDenominator : generating 1M decays to remove the \"phasespace\" "
               "distribution..."
            << std::endl;
    ...
    if ( part->nDaughters() > 0 ) {
      ...
      if ( !decay_.SetDecay( part->getP(), part->nDaughters(), part->daughterMasses() ) ) {
        if ( !suppressKinematicWarning_ ) {
          std::cout << "WARNING in RapidDecay::genDecay : decay of " << part->name()
                    << " is kinematically forbidden for some events due to resonance mass shapes." << std::endl
      ...
      if ( acceptAny ) {
        ...
        if ( !accept ) {
          if ( !suppressAttemptsWarning_ ) {
            std::cout << "WARNING in RapidDecay::genDecay : rejected all " << maxgen_ << " attempts to decay "
                      << part->name() << "." << std::endl
