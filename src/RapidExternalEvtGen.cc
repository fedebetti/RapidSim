...
bool RapidExternalEvtGen::decay( std::vector<RapidParticle*>& parts ) {
  ...
  while ( !evtParts.empty() ) {
    ...
    while ( theParticle->getNDaug() == 1 ) { theParticle = theParticle->getDaug( 0 ); }
...
#else
  if ( !suppressWarning_ ) {
    std::cout << "WARNING in RapidExternalEvtGen::decay : EvtGen extension not compiled. Will not use EvtGen to decay "
              << parts[0]->name() << "." << std::endl;
...
bool RapidExternalEvtGen::setupGenerator() {
  ...
  evtGen_ = new EvtGen( decPath.Data(), evtPDLPath.Data(), randomEngine, radCorrEngine, &extraModels );
...
void RapidExternalEvtGen::writeDecFile( TString fname, std::vector<RapidParticle*>& parts, bool usePhotos ) {
  ...
  for ( unsigned int iPart = 0; iPart < parts.size(); ++iPart ) {
    ...
    if ( nChildren > 0 ) {
      ...
      if ( TMath::Abs( id ) == 531 || TMath::Abs( id ) == 511 )
        fout << "CDecay " << getEvtGenConjName( id ) << std::endl << std::endl;
    }
  }
  fout << "End\n" << std::endl;
  fout.close();
...
TString RapidExternalEvtGen::getEvtGenName( int id ) {
  ...
  return name;
#else
  std::cout << "WARNING in RapidExternalEvtGen::getEvtGenName : EvtGen extension not compiled. Cannot lookup name for "
               "particle ID "
            << id << "." << std::endl;
  return "";
#endif
...
TString RapidExternalEvtGen::getEvtGenConjName( int id ) {
  ...
  return name;
#else
  std::cout << "WARNING in RapidExternalEvtGen::getEvtGenConjName : EvtGen extension not compiled. Cannot lookup "
               "conjugate name for particle ID "
            << id << "." << std::endl;
  return "";
#endif
}
